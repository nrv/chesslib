...
public class GameLoader {
	...
	/**
	 * Loads the next game of chess from an iterator over the lines of a Portable Game Notation (PGN) file. The iteration ends when the game is fully loaded, hence the iterator is not consumed more than necessary.
	 *
	 * @param iterator
	 *            the iterator over the lines of a PGN file
			...
			try {
				if (isProperty(line)) {
					try {
						...
						container.game.setExceptionInParsing(e);
						return container.game;
					}
				} else if (StringUtils.isNotEmpty(line)) {
					addMoveText(line, container);
					if (isEndGame(line)) {
						if (!Game.isParsable(container.game)) {
							return container.game;
						}
						try {
	...
	private static void addProperty(String line, PgnTempContainer container) throws Exception {
		...
		case "variation":
			...
			break;
		case "variant":
			container.game.setVariant(property.value);
			break;
		case "whiteelo":
			try {
